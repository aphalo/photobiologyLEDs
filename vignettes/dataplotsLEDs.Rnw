...
\title{\PBLam Version 0.1.2\\ Catalogue of LEDs}
...
opts_chunk$set(fig.path='figure/pos-', fig.align='center', fig.show='hold', size="footnotesize",
               fig.width=7, fig.height=4, out.width='.9\\textwidth', dev='cairo_pdf')
@
...
print(plot(a.spct, unit="energy") + labs(title=object.name) + theme_grey(10))
# print(plot(a.spct, unit="photon") + labs(title=object.name) + theme_grey(10))
}
@
...
<<plot-TaoYuan,eval=FALSE>>=
TY.UV.LEDs <- c("")
for (lamp in TYL.UV.LEDs) {
  lamp.plotter(lamp.name=lamp)
}
@
