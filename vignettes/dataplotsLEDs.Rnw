...
<<plot-RGB-Norlux>>=
Norlux.spct <- rbindspct(list(R = Norlux_R.spct, G = Norlux_G.spct, B = Norlux_B.spct),
                         idfactor = "channel")
figNorlux <- ggplot(Norlux.spct, aes(w.length, s.e.irrad, colour = channel)) +
  geom_line()
figNorlux
add_color_guide(figNorlux)
@
